...
```{r create_rainfall_recent_tidy, paged.print=FALSE}
rainfall_recent_tidy <- 
  ...
  mutate(city = factor(city) %>% fct_inorder()) %>%
  mutate(year = factor(year) %>% fct_inseq())
...
### Listing 10.17. Redefinition of the `x` values as the cities instead of the years, where the `fill` is now of the `year`.
...
ggplot(rainfall_recent_tidy, aes(x = city, y = precip, fill = year)) +
  ...
  annotate("text", x = 0.77, y = 1270, label = "2017", angle = 90, size = 3) +
  annotate("text", x = 1.00, y = 1420, label = "2018", angle = 90, size = 3) +
  annotate("text", x = 1.24, y = 1030, label = "2019", angle = 90, size = 3) +
  theme_minimal() +
  theme(
