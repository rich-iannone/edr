...
First, let's get a subset of the employment data using **dplyr**'s `slice_tail()` function (it gets the last *n* rows of a table). The transformed dataset, named `employ_recent`, is now used to make a new line plot (*Figure \@ref(fig:employ-recent-two-lines)*). The key difference here is that we will make two different lines: (1) a solid line using values in the employed column, with points overlaid on top of that line, (2) a dashed line using values in the nonagriculture column. The line with styled points is made possible with a `geom_point()` statement called afterward, taking advantage of the layering that **ggplot** provides.
